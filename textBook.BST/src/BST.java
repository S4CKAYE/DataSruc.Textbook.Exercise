...
public class BST {
	...
	//IMPLEMENTED
	//return integer in the tree that is lower than n
	public int prevOfN(int n) {
		BSTNode current = root;
		int prev = Integer.MIN_VALUE;
		
		if (findMin(root).currentNode.data >= n) {
			return -1;
		}
		//at this point, it is certain that there is an integer less than n.
	...
	public int nextOfN(int n) {
		BSTNode current = root;
		int next = Integer.MAX_VALUE;
		TreeIterator maxItr = (TreeIterator)findMax();
		if (n >= maxItr.currentNode.data) {
			return -1;
		}
		//at this point, it is certain that there is a node having value greater than n.
		while(current != null) {
			if (current.data > n) {
	...
	// *this method uses Arraylist. 
	// *also has recursive version written in BSTRecursive
	...
	// *this method uses arrayList/ also could use stack
	public int numNodes() {
		return size;
	}
	...
	//this method combines this with binary tree t
	//result is that "this" with all data from t also inside it.
	//uses breath-first traversal using method addDataByLevel()
	public void combine(BST t) {
		QueueArray dataByLevelofT = addDataByLevel(t.root);
		try {
		while(!dataByLevelofT.isEmpty()) {
			int v = ((BSTNode)(dataByLevelofT.removeFirst())).data;
			this.insert(v);
		}
		} catch (Exception e) {
			return;
	...
	//add node in a tree level by level, start from root
	private QueueArray addDataByLevel(BSTNode root) {
		QueueArray thisLevel = new QueueArray();
		QueueArray nextLevel = new QueueArray();
		QueueArray value = new QueueArray(); 
		try {
		thisLevel.insertLast(root);
		while(!thisLevel.isEmpty() || !nextLevel.isEmpty()) {
			if(!thisLevel.isEmpty()) {
				BSTNode current = (BSTNode) thisLevel.removeFirst();
				if(current.left != null) {
	...
	public static void main(String[] args) {
		...
		//System.out.println("\n=====Test: prevOfN=====\n");
		//System.out.println(t.prevOfN(7));
		//System.out.println("\n=====Test: nextOfN=====\n");
		//System.out.println(t.nextOfN(10));
//		System.out.println("\n=====Test: numLeaves=====\n");
